/**
  ...
  * @version V1.0
  * @date    2016-xx-xx
  * @brief   MLCD应用函数接口，支持ARGB888/RGB888/RGB565/ARGB4444/ARGB1555 (不含中文显示)
  ******************************************************************************
  * @attention
  *
...
/* Default MLCD configuration with MLCD Layer 1 */
static uint32_t            ActiveLayer = 0;
static MLCD_DrawPropTypeDef DrawProp[MAX_LAYER_NUMBER];
/**
 * @brief  Initializes the MLCD.
...
static void LL_ConvertLineToARGB8888(void * pSrc, void *pDst, uint32_t xSize, uint32_t ColorMode);
  ...
  * @retval 无
  */
static void MLCD_GPIO_Config(void)
{ 
...
void MLCD_ClockConfig(void)
{
  ...
  static RCC_PeriphCLKInitTypeDef  periph_clk_init_struct;
	...
	// AMPIRE640480 MLCD clock configuration 
	// PLL3_VCO Input = HSE_VALUE/PLL3M = 1 Mhz 
	// PLL3_VCO Output = PLL3_VCO Input * PLL3N = 270 Mhz 
	// PLLMLCDCLK = PLL3_VCO Output/PLL3R = 270/10 = 27Mhz 
	// LTDC clock frequency = PLLMLCDCLK = 27 Mhz   
...
{ 
    ...
    /* 配置(HSW+HBP+有效像素宽度-1) */
    Ltdc_Handler.Init.AccumulatedActiveW = HSW+HBP+MLCD_PIXEL_WIDTH-1;
    /* 配置(VSW+VBP+有效像素高度-1) */
    Ltdc_Handler.Init.AccumulatedActiveH = VSW+VBP+MLCD_PIXEL_HEIGHT-1;
    /* 配置总宽度(HSW+HBP+有效像素宽度+HFP-1) */
    Ltdc_Handler.Init.TotalWidth =HSW+ HBP+MLCD_PIXEL_WIDTH + HFP-1; 
    /* 配置总高度(VSW+VBP+有效像素高度+VFP-1) */
    Ltdc_Handler.Init.TotalHeigh =VSW+ VBP+MLCD_PIXEL_HEIGHT + VFP-1;

    /* 初始化MLCD的像素宽度和高度 */
    Ltdc_Handler.LayerCfg->ImageWidth  = MLCD_PIXEL_WIDTH;
    Ltdc_Handler.LayerCfg->ImageHeight = MLCD_PIXEL_HEIGHT;
    /* 设置MLCD背景层的颜色，默认黑色 */
    ...
    /* 初始化字体 */
    MLCD_SetFont(&MLCD_DEFAULT_FONT);


    MLCD_LayerInit(0, (uint32_t)&_lay0[0],ARGB8888);
	  MLCD_LayerInit(1, (uint32_t)&_lay1[0],ARGB8888);
	  MLCD_DisplayOn(); 
}

/**
  * @brief  获取MLCD当前有效层X轴的大小
  * @retval X轴的大小
  */
...
{     
  ...
  layer_cfg.WindowX1 = MLCD_GetXSize();	//窗口结束位置X坐标
  layer_cfg.WindowY0 = 0;				//窗口起始位置Y坐标
  layer_cfg.WindowY1 = MLCD_GetYSize();  //窗口结束位置Y坐标
  ...
  layer_cfg.ImageWidth = MLCD_GetXSize();//设置图像宽度
  layer_cfg.ImageHeight = MLCD_GetYSize();//设置图像高度
  
  HAL_LTDC_ConfigLayer(&Ltdc_Handler, &layer_cfg, LayerIndex); //设置选中的层参数

  DrawProp[LayerIndex].BackColor = MLCD_COLOR_WHITE;//设置层的字体颜色
  DrawProp[LayerIndex].pFont     = &MLCD_DEFAULT_FONT;//设置层的字体类型
  DrawProp[LayerIndex].TextColor = MLCD_COLOR_BLACK; //设置层的字体背景颜色
...
/**
  * @brief  设置显示窗口
  * @param  LayerIndex: 前景层(层1)或者背景层(层0)
  * @param  Xpos: MLCD 	X轴起始位置
  * @param  Ypos: MLCD 	Y轴起始位置
  * @param  Width: MLCD  窗口大小
  * @param  Height: MLCD 窗口大小  
  * @retval None
  */
...
void MLCD_SetColors(uint32_t TextColor, uint32_t BackColor)
{
     ...
     MLCD_SetBackColor (BackColor);
}
/**
  * @brief  设置MLCD当前层显示的字体
  * @param  fonts: 字体类型
  * @retval None
  */
...
/**..\..\Output\Fire_uCOS.axf: Error: L6218E: Undefined symbol LCD_GetFont (referred from bsp_lcd.o).
  * @brief  读MLCD的像素值
  * @param  Xpos: X 轴坐标位置
  * @param  Ypos: Y 轴坐标位置
  * @retval RGB像素值
...
uint32_t MLCD_ReadPixel(uint16_t Xpos, uint16_t Ypos)
{
  ...
  {
    /* 从SDRAM显存中读取颜色数据 */
    ret = *(__IO uint32_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2*(Ypos*MLCD_GetXSize() + Xpos)));
  }
  else if(Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
    /* 从SDRAM显存中读取颜色数据 */
    ret  = (*(__IO uint8_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3*(Ypos*MLCD_GetXSize() + Xpos))+2) & 0x00FFFFFF);
	ret |= (*(__IO uint8_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3*(Ypos*MLCD_GetXSize() + Xpos))+1) & 0x00FFFFFF);
	ret |= (*(__IO uint8_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3*(Ypos*MLCD_GetXSize() + Xpos))) & 0x00FFFFFF);
  }
  ...
  {
    /* 从SDRAM显存中读取颜色数据 */
    ret = *(__IO uint16_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2*(Ypos*MLCD_GetXSize() + Xpos)));    
  }
  else
  {
    /* 从SDRAM显存中读取颜色数据 */
    ret = *(__IO uint8_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2*(Ypos*MLCD_GetXSize() + Xpos)));    
  }
...
{ 
  /* 清屏 */ 
  LL_FillBuffer(ActiveLayer, (uint32_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress), MLCD_GetXSize(), MLCD_GetYSize(), 0, Color);
}
...
void MLCD_ClearLine(uint32_t Line)
{
  ...
  /* 画一个跟背景色一致的矩形完成清除 */
  MLCD_FillRect(0, (Line * DrawProp[ActiveLayer].pFont->Height), MLCD_GetXSize(), DrawProp[ActiveLayer].pFont->Height);
  
  DrawProp[ActiveLayer].TextColor = color_backup;
  MLCD_SetTextColor(DrawProp[ActiveLayer].TextColor);  
	HAL_Delay(1);
}
...
void MLCD_DisplayStringAtABSPos(uint16_t Xpos, uint16_t Ypos, uint8_t *Text)
{
  ...
  /* 使用字符显示函数显示每一个字符*/
  while ((*Text != 0) & (((MLCD_GetXSize() - (i*DrawProp[ActiveLayer].pFont->Width)) & 0xFFFF)\
			>= DrawProp[ActiveLayer].pFont->Width))
  {
    /* 显示一个字符 */
    MLCD_DisplayChar(ref_column, Ypos, *Text);
    /* 根据字体大小计算下一个偏移位置 */
...
void MLCD_DisplayStringAt(uint16_t Xpos, uint16_t Ypos, uint8_t *Text, Text_AlignModeTypdef Mode)
{
  ...
  /* 每一行可以显示字符的数量 */
  xsize = (MLCD_GetXSize()/DrawProp[ActiveLayer].pFont->Width);
...
{  
  MLCD_DisplayStringAt(0, LINE(Line), ptr, LEFT_MODE);
}
...
void MLCD_DrawHLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  ...
  if(Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB8888)
  {
    Xaddress = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + 4*(MLCD_GetXSize()*Ypos + Xpos);
  }
  else if(Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
    Xaddress = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + 3*(MLCD_GetXSize()*Ypos + Xpos);
  }
  else if((Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB565) || \
          (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB4444) || \
          (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_AL88))  
  {
    Xaddress = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + 2*(MLCD_GetXSize()*Ypos + Xpos);   
  }
  else
  {
    Xaddress = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + 2*(MLCD_GetXSize()*Ypos + Xpos);   
...
void MLCD_DrawVLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  ...
  /* 填充数据 */
  LL_FillBuffer(ActiveLayer, (uint32_t *)Xaddress, 1, Length, (MLCD_GetXSize() - 1), DrawProp[ActiveLayer].TextColor);
}
...
void MLCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
  ...
  {
    MLCD_DrawPixel(x, y, DrawProp[ActiveLayer].TextColor);   /* 绘制当前像素点 */
...
{
  /* 绘制水平线 */
  MLCD_DrawHLine(Xpos, Ypos, Width);
  MLCD_DrawHLine(Xpos, (Ypos+ Height), Width);
  
  /* 绘制垂直线 */
  MLCD_DrawVLine(Xpos, Ypos, Height);
  MLCD_DrawVLine((Xpos + Width), Ypos, Height);
}
...
void MLCD_DrawCircle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius)
{
  ...
  {
    MLCD_DrawPixel((Xpos + current_x), (Ypos - current_y), DrawProp[ActiveLayer].TextColor);
    
    MLCD_DrawPixel((Xpos - current_x), (Ypos - current_y), DrawProp[ActiveLayer].TextColor);
    
    MLCD_DrawPixel((Xpos + current_y), (Ypos - current_x), DrawProp[ActiveLayer].TextColor);
...
void MLCD_DrawPolygon(pPoint Points, uint16_t PointCount)
{
  ...
  MLCD_DrawLine(Points->X, Points->Y, (Points+PointCount-1)->X, (Points+PointCount-1)->Y);
  ...
  while(--PointCount)
  {
    ...
    y = Points->Y;
    Points++;
    MLCD_DrawLine(x, y, Points->X, Points->Y);
  }
}
...
void MLCD_DrawEllipse(int Xpos, int Ypos, int XRadius, int YRadius)
{
  ...
  do { 
    MLCD_DrawPixel((Xpos-(uint16_t)(x/k)), (Ypos+y), DrawProp[ActiveLayer].TextColor);
    MLCD_DrawPixel((Xpos+(uint16_t)(x/k)), (Ypos+y), DrawProp[ActiveLayer].TextColor);
    MLCD_DrawPixel((Xpos+(uint16_t)(x/k)), (Ypos-y), DrawProp[ActiveLayer].TextColor);
    MLCD_DrawPixel((Xpos-(uint16_t)(x/k)), (Ypos-y), DrawProp[ActiveLayer].TextColor);      
...
/**
  ...
  * @retval 无
  */
void MLCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint32_t RGB_Code)
{

  if(Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB8888)
  {
    *(__IO uint32_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (4*(Ypos*MLCD_GetXSize() + Xpos))) = RGB_Code;
  }
  else if(Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
    *(__IO uint8_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3*(Ypos*MLCD_GetXSize() + Xpos))+2) = 0xFF&(RGB_Code>>16);
		*(__IO uint8_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3*(Ypos*MLCD_GetXSize() + Xpos))+1) = 0xFF&(RGB_Code>>8);
		*(__IO uint8_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3*(Ypos*MLCD_GetXSize() + Xpos))) = 0xFF&RGB_Code;
  }
  else if((Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB565) || \
          (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB4444) || \
          (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_AL88))  
  {
    *(__IO uint16_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2*(Ypos*MLCD_GetXSize() + Xpos))) = (uint16_t)RGB_Code;   
  }
  else
  {
    *(__IO uint16_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2*(Ypos*MLCD_GetXSize() + Xpos))) = (uint16_t)RGB_Code;   
...
void MLCD_DrawBitmap(uint32_t Xpos, uint32_t Ypos, const uint8_t *pbmp)
{
  ...
  /* 设定地址 */
  address = Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (((MLCD_GetXSize()*Ypos) + Xpos)*(4));
...
void MLCD_FillRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
  ...
  /* 设置矩形开始地址 */
    if(Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB8888)
  {
    x_address = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + 4*(MLCD_GetXSize()*Ypos + Xpos);
  }
  else if(Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
    x_address = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + 3*(MLCD_GetXSize()*Ypos + Xpos);
  }
  else if((Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB565) || \
          (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB4444) || \
          (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_AL88))  
  {
    x_address = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + 2*(MLCD_GetXSize()*Ypos + Xpos);   
  }
  else
  {
    x_address = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + 2*(MLCD_GetXSize()*Ypos + Xpos);
  ...
  /* 填充矩形 */
  LL_FillBuffer(ActiveLayer, (uint32_t *)x_address, Width, Height, (MLCD_GetXSize() - Width), DrawProp[ActiveLayer].TextColor);
}
...
void MLCD_FillCircle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius)
{
  ...
  while (current_x <= current_y)
  {
    if(current_y > 0) 
    {
      MLCD_DrawHLine(Xpos - current_y, Ypos + current_x, 2*current_y);
      MLCD_DrawHLine(Xpos - current_y, Ypos - current_x, 2*current_y);
    }
    
    if(current_x > 0) 
    {
      MLCD_DrawHLine(Xpos - current_x, Ypos - current_y, 2*current_x);
      MLCD_DrawHLine(Xpos - current_x, Ypos + current_y, 2*current_x);
    }
    if (decision < 0)
    { 
  ...
  MLCD_SetTextColor(DrawProp[ActiveLayer].TextColor);
  MLCD_DrawCircle(Xpos, Ypos, Radius);
}
...
void MLCD_FillEllipse(int Xpos, int Ypos, int XRadius, int YRadius)
{
  ...
  do 
  {       
    MLCD_DrawHLine((Xpos-(uint16_t)(x/k)), (Ypos+y), (2*(uint16_t)(x/k) + 1));
    MLCD_DrawHLine((Xpos-(uint16_t)(x/k)), (Ypos-y), (2*(uint16_t)(x/k) + 1));
...
static void DrawChar(uint16_t Xpos, uint16_t Ypos, const uint8_t *c)
{
  ...
  for(i = 0; i < height; i++)//遍历字体高度绘点
  {
    ...
    for (j = 0; j < width; j++)//遍历字体宽度绘点
    {
      ...
      {
        MLCD_DrawPixel((Xpos + j), Ypos, DrawProp[ActiveLayer].TextColor);
      }
      else//如果这一行没有字体像素则按照背景颜色绘点
      {
        MLCD_DrawPixel((Xpos + j), Ypos, DrawProp[ActiveLayer].BackColor);
      } 
    }
    Ypos++;
  }
