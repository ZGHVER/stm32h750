...
void GTP_TouchRegisterCallBack(TOUCH_UP touch_upCallback, TOUCH_DOWN touch_DownCallback){
    _touch_up = touch_upCallback;
    _touch_down = touch_DownCallback;
}
...
//    GTP_DEBUG_FUNC();
    ...
    if (touch_num)
    {
        for (i = 0; i < touch_num; i++)						//一个点一个点处理
        {
            ...
            {
                _touch_down( id, input_x, input_y, input_w);//数据处理
            }
        }
    }
...
void VOIOFUN(int32_t i){
    
};

void VOIOFUN1(int32_t i,int32_t j,int32_t k,int32_t g){
...
//		GTP_DEBUG_ARRAY(config,6);
		...
		/*根据LCD的扫描方向设置分辨率*/
		config[GTP_ADDR_LENGTH+1] = MLCD_PIXEL_WIDTH & 0xFF;
		config[GTP_ADDR_LENGTH+2] = MLCD_PIXEL_WIDTH >> 8;
		config[GTP_ADDR_LENGTH+3] = MLCD_PIXEL_HEIGHT & 0xFF;
		config[GTP_ADDR_LENGTH+4] = MLCD_PIXEL_HEIGHT >> 8;
